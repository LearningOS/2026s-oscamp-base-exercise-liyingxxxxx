...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let pte = self.nodes.get(&current_ppn).unwrap().entries[index];
        ...
        let leaf_index = Self::extract_vpn(va, 0);
        self.nodes.get_mut(&current_ppn).unwrap().entries[leaf_index] =
            (pa_ppn << PPN_SHIFT) | flags;
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let level2_index = Self::extract_vpn(va, 2);
        let root_pte = self.nodes.get(&self.root_ppn).unwrap().entries[level2_index];
        let level1_ppn = if root_pte & PTE_V != 0 {
            root_pte >> PPN_SHIFT
        } else {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&self.root_ppn).unwrap().entries[level2_index] =
                (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        };

        let level1_index = Self::extract_vpn(va, 1);
        self.nodes.get_mut(&level1_ppn).unwrap().entries[level1_index] =
